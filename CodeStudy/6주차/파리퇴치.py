...
def replaceData(numData): # numData	2차원 정수 배열
    ...
    ###########   여기부터 코딩 (1) ---------------->
    retData = numData[:] #이걸 몰라서 못풀었네.. 2차원 배열 가져오기

    for i in range(0, ARRAY_LENGTH) :
        for k in range(0, ARRAY_LENGTH) : 
                if(numData[i][k] < 0): #배열값이 음수면 0으로
                        numData[i][k] = 0
                if(numData[i][k] > 100): #배열값이 100 이상이면 100으로 나눈 나머지로
                        numData[i][k] %= 100
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    for i in range(ARRAY_LENGTH-1):
            for k in range(ARRAY_LENGTH-1): #행과 열의 길이가 배열을 넘어가지 않게 '-1'로 조정
                hap = numData[i][k] + numData[i][k+1] + numData[i+1][k] + numData[i+1][k+1] #각각 4칸의 값을 더함
                if hap > maxSum: #더한 값이 최대 합이라면
                        maxSum = hap #최대합 갱신

    ###########   <-------------- 여기까지 코딩 (2)

    return maxSum #최대합 반환
